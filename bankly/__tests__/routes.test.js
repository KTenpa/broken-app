// Fix for pg + Jest compatibility: TextEncoder/TextDecoder not defined in Node test env
...
describe("POST /auth/login", function() {
  ...
  // TESTS BUG #1
  test("should fail properly if password is incorrect", async function() {
    const response = await request(app)
      .post("/auth/login")
...
describe("GET /users/[username]", function() {
  ...
  // TESTS BUG #3
  test("should 404 if user not found", async function() {
    const response = await request(app)
...
describe("DELETE /users/[username]", function() {
  ...
  // TESTS BUG #2
  test("should 404 if trying to delete a non-existent user", async function() {
    const response = await request(app)
...
describe("AUTH Middleware", function() {
  test("should deny access if token is invalid", async function() {
    const badToken = "this.is.not.valid.jwt";
    const response = await request(app)
      .get("/users")
